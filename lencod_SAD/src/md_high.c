...
void encode_one_macroblock_high (Macroblock *currMB)
{
  ...
  //enc_mb.lambda_md = enc_mb.lambda_me[0];
  //enc_mb.lambda_mdfp = enc_mb.lambda_mf[0];
  enc_mb.lambda_md = 1.2 * enc_mb.lambda_me[0]; // ^{  ^n   ^x1.1
  enc_mb.lambda_mdfp = (int)((double)1.2 * (double)(enc_mb.lambda_mf[0])); //int   double   int
    ...
    for (mode = 1; mode < 4; mode++)
    {
      ...
      if (enc_mb.valid[mode])
      {
        for (cost=0, block=0; block<(mode==1?1:2); block++)
        {
          ...
          PartitionMotionSearch (currMB, mode, block, lambda_mf); //做了一次ME

          // === for debug only 打印出所有Lambda ===
